...
- **Accessibility and SEO are hard assertions at a perfect score.** They barely move between runs, so there is no reason to accept less. Best practices sits at `warn` for now, and only because `errors-in-console` catches the Live Content API stream failing CORS on a preview: the per-deployment URL is not one of the Sanity project's allowed origins. Add `https://*.vercel.app` there and put it back to `error` — the audit itself is worth failing on.
