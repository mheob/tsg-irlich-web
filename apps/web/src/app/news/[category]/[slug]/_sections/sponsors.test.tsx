...
describe('the sponsors beside an article', () => {
	it('heads the list with its own title', () => {
		const { getByRole } = renderWithUser(<Sponsors sponsors={buildSponsors([sponsor('one')])} />);

		expect(getByRole('heading', { name: 'Partner' })).not.toBeNull();
	});
