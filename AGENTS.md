...
- `pnpm run test:coverage` writes `coverage/lcov.info` per workspace for CI/SonarQube; no minimum coverage threshold yet. Every `vitest.config.ts` sets `coverage.include` so untested files count as uncovered instead of dropping out of the denominator — Vitest 4 removed `coverage.all`, and without `include` V8 only scores the files a test happened to import, which inflated every figure. The honest numbers as of the change: `packages/shared` and `packages/email` 100% lines, `apps/web` 52.7%, `apps/studio` 23.3%
