...
- `pnpm run test:coverage` writes `coverage/lcov.info` per workspace for CI/SonarQube; no minimum coverage threshold yet. Read those percentages with care: no `vitest.config.ts` sets `coverage.all`, so V8 only scores files a test actually imported — an untested file is missing from the denominator rather than counted as uncovered, which inflates every figure (`packages/shared` and `packages/email` report 100% while several files are simply invisible). Turn `coverage.all` on in all four configs and re-measure before quoting a number or setting a threshold
